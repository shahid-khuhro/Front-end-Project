...
const handleOutgoingMessage = (e, fromInitial = false) => {
  ...
  const inputElement = fromInitial ? initialInput : messageInput
  userData.message = inputElement.value.trim()
  ...
  // Transition to full chat view on first message
  if (isFirstMessage) {
  ...
  const messageContent = `<div class="message-text"></div>
                    ${userData.file.data ? `<img src="data:${userData.file.mime_type};base64,${userData.file.data}" class="attachment" alt="Attachment" />` : ""}`
...
initialForm.addEventListener("submit", (e) => handleOutgoingMessage(e, true))

initialInput.addEventListener("keydown", (e) => {
  const userMessage = e.target.value.trim()
  if (e.key === "Enter" && !e.shiftKey && userMessage) {
    e.preventDefault()
    handleOutgoingMessage(e, true)
  }
})
...
document.querySelector(".emoji-picker-wrapper").appendChild(picker)
